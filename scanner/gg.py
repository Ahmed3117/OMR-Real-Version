    ...
    for filename in os.listdir(folder_path):
        ...
        # Check if the file is an image file (you may need to modify this to match your file extensions)
            if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".PNG") or filename.endswith(".gif"):
                ...
                middle_half = lower_input_image.crop((split_coord, 0, split_coord*2, height))
                right_half = lower_input_image.crop((split_coord*2, 0, width, height))
                # Convert the images to RGB mode
                left_half = left_half.convert('RGB')
                middle_half = middle_half.convert('RGB')
                right_half = right_half.convert('RGB')

                # Save the two halves as separate files
                left_half.save(new_folder_path+"/left_"+filename[:-4]+".jpg")
                middle_half.save(new_folder_path+"/middle_"+filename[:-4]+".jpg")
                ...
                h = int(round(250 * left_image.shape[0] / left_image.shape[1]))
                frame = cv2.resize(left_image, (250, h), interpolation=cv2.INTER_LANCZOS4)
                ...
                h = int(round(250 * right_image.shape[0] / right_image.shape[1]))
                frame = cv2.resize(right_image, (250, h), interpolation=cv2.INTER_LANCZOS4)
                cannyFrame = getCannyFrame(frame)
                warpedFrame = getWarpedFrame(cannyFrame, frame)
                adaptiveFrame = getAdaptiveThresh(warpedFrame)
                        ...
                        for (j, c) in enumerate(bubbles):
                            ...
                            if (isBubbleSigned):
                                if (answer == j):
                                    # And calculate score
                                    score += 1
                middle_image = cv2.imread(new_folder_path+"/middle_"+filename[:-4]+".jpg")
                h = int(round(250 * middle_image.shape[0] / middle_image.shape[1]))
                frame = cv2.resize(middle_image, (250, h), interpolation=cv2.INTER_LANCZOS4)
                cannyFrame = getCannyFrame(frame)
                warpedFrame = getWarpedFrame(cannyFrame, frame)
                adaptiveFrame = getAdaptiveThresh(warpedFrame)
