...
score = 0
secretenumber = ''
dimentions = [400,300,280,310,250,290,200,600,220,320]
upperdimentions=[600,400,500,420,300,320]
failed_images = []
...
def printscore(request):
    ...
    ANSWER_KEY1 = {}
    ANSWER_KEY2 = {}
    for box in range(0,int(questionCount/2)):
    ...
    print(ANSWER_KEY1)
    for box in range(int(questionCount/2),questionCount):
        ans=request.POST.get("box-"+str(box)+"-radio")
        if ans == None :
            ANSWER_KEY2[box-int(questionCount/2)] = ans
        else:
            ANSWER_KEY2[box-int(questionCount/2)] = int(ans)
    print(ANSWER_KEY2)
    ...
    for filename in os.listdir(uploaded_files_path): 
        if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".PNG") :
            try:
                ...
                ovalcontours_from_image_with_normal_dimntions = get_ovalcontours_from_image_with_normal_dimntions(left_image_path)
                ovalContours = ovalcontours_from_image_with_normal_dimntions[0]
                adaptiveFrame = ovalcontours_from_image_with_normal_dimntions[1]
                print("left without dimentionsssssssssssssssssssssssssssssssssssss")
                ...
                if (len(ovalContours) == ovalCount/2):
                    ...
                    for dim in dimentions:
                        ovalcontours_from_image_with_list_of_dimentions=get_ovalcontours_from_image_with_list_of_dimentions(left_image_path,dim)
                ...
                if (len(ovalContours) == ovalCount/2):
                    ...
                    for dim in dimentions:
                        ovalcontours_from_image_with_list_of_dimentions=get_ovalcontours_from_image_with_list_of_dimentions(right_image_path,dim)
                        ovalContours = ovalcontours_from_image_with_list_of_dimentions[0]
                        adaptiveFrame = ovalcontours_from_image_with_list_of_dimentions[1]
                        print('dim => '+ str(dim) +'=> ' + str(len(ovalContours)))
                        if (len(ovalContours) == ovalCount/2):
                ...
                ovalcontours_from_image_with_normal_dimntions = get_ovalcontours_from_image_with_normal_dimntions(upper_image_path)
                ovalContours = ovalcontours_from_image_with_normal_dimntions[0]
                adaptiveFrame = ovalcontours_from_image_with_normal_dimntions[1]
