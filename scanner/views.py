...
#     gray = cv2.GaussianBlur(gray, (1, 1), 0) # change 3,3 to 5,5
#     frame = cv2.Canny(gray, 127, 255) # change 100, 200 to 75, 200
#     return frame
...
def printscore(request):
    ...
    for filename in os.listdir(uploaded_files_path): 
        if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".PNG") or filename.endswith(".pdf") or filename.endswith(".PDF") :
            try:
                print(filename)
                thresholding_image(uploaded_files_path,filename,100)
                ...
                for dim in upperdimentions:
                    ...
                    # upper_image_path = cv2.imread(upper_image_path)
                    # h, w, _ = upper_image_path.shape
                    # frame = cv2.resize(upper_image_path, (w, h), interpolation=cv2.INTER_LANCZOS4)
