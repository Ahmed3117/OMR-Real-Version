    ...
    for filename in os.listdir(folder_path):
        ...
        # Check if the file is an image file (you may need to modify this to match your file extensions)
            if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".PNG") or filename.endswith(".gif"):
                ...
                middle_left_half = lower_input_image.crop((split_coord, 0, split_coord*2, height))
                middle_right_half = lower_input_image.crop((split_coord*2, 0, split_coord*3, height))
                right_half = lower_input_image.crop((split_coord*3, 0, width, height))
                # Convert the images to RGB mode
                left_half = left_half.convert('RGB')
                middle_left_half = middle_left_half.convert('RGB')
                middle_right_half = middle_right_half.convert('RGB')
                right_half = right_half.convert('RGB')

                # Save the two halves as separate files
                left_half.save(new_folder_path+"/left_"+filename[:-4]+".jpg")
                middle_left_half.save(new_folder_path+"/middle_left_"+filename[:-4]+".jpg")
                middle_right_half.save(new_folder_path+"/middle_right_"+filename[:-4]+".jpg")
                ...
                def scoreExam(image,expected_contours_number):
                    dimentions = [320,300,600,310,250,290,200,]
                    image = cv2.imread(image)
                    # h = int(round(300 * image.shape[0] / image.shape[1]))
                    # frame = cv2.resize(image, (300, h), interpolation=cv2.INTER_LANCZOS4)
                    cannyFrame = getCannyFrame(image)
                    warpedFrame = getWarpedFrame(cannyFrame, image)
                    adaptiveFrame = getAdaptiveThresh(warpedFrame)
                    ovalContours = getOvalContours(adaptiveFrame)
                    print(len(ovalContours))
                    
                    if (len(ovalContours) == expected_contours_number):
                        print("left----------------" )
                        print(len(ovalContours))
                        ...
                        for dim in dimentions:
                            ...
                            h = int(round(dim * image.shape[0] / image.shape[1]))
                            frame = cv2.resize(image, (dim, h), interpolation=cv2.INTER_LANCZOS4)
                    ...
                    return len(ovalContours)


                scoreExam(new_folder_path+"/left_"+filename[:-4]+".jpg",100)
                scoreExam(new_folder_path+"/right_"+filename[:-4]+".jpg",100)
                scoreExam(new_folder_path+"/middle_left_"+filename[:-4]+".jpg",100)
                scoreExam(new_folder_path+"/middle_right_"+filename[:-4]+".jpg",100)
                scoreExam(new_folder_path+"/upper_"+filename[:-4]+".jpg",upperovalCount)
